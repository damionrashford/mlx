...
description: >
  ...
  segmentation, trend detection, business metrics, pre-delivery validation,
  and data visualization. Use when the user asks to "analyze this data",
  "run a statistical test", "compare groups", "find trends", "do A/B test
  analysis", "segment customers", "calculate KPIs", "validate this analysis",
  "check my work", "sanity check", "review my numbers", "make a chart",
  "create a dashboard", "plot the data", "visualize results", or mentions
  hypothesis testing, cohort analysis, business analytics, data validation,
  bar charts, line charts, heatmaps, scatter plots, or data storytelling.
allowed-tools: >
  Bash(uv run * scripts/descriptive_stats.py *)
  Bash(uv run * scripts/hypothesis_test.py *)
  Bash(uv run * scripts/ab_test.py *)
  Bash(uv run * scripts/cohort_analysis.py *)
  Bash(uv run * scripts/rfm_segmentation.py *)
  Bash(uv run * scripts/trend_analysis.py *)
  Bash(uv run * scripts/validate.py *)
  Bash(uv run * scripts/chart_templates.py *)
  Bash(uv run * scripts/format_number.py *)
  Read Write Glob Grep
argument-hint: path to dataset or description of analysis/chart (e.g. "data/sales.csv" or "bar chart of revenue by quarter")
model: sonnet
effort: medium
compatibility: ">=1.0"
metadata:
  category: data-analysis
  tags: [statistics, hypothesis-testing, a/b-testing, visualization, kpi, segmentation, cohort, eda]
  phase: explore
---
...
| Script                                               | Usage                                                                                                                      |
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| [descriptive_stats.py](scripts/descriptive_stats.py) | `uv run ${CLAUDE_SKILL_DIR}/scripts/descriptive_stats.py data.csv --group segment --value revenue`                         |
| [hypothesis_test.py](scripts/hypothesis_test.py)     | `uv run ${CLAUDE_SKILL_DIR}/scripts/hypothesis_test.py data.csv --col value --group segment --a control --b treatment`     |
| [ab_test.py](scripts/ab_test.py)                     | `uv run ${CLAUDE_SKILL_DIR}/scripts/ab_test.py data.csv --col converted --group variant --control A --treatment B`         |
| [cohort_analysis.py](scripts/cohort_analysis.py)     | `uv run ${CLAUDE_SKILL_DIR}/scripts/cohort_analysis.py data.csv --user user_id --date order_date`                          |
| [rfm_segmentation.py](scripts/rfm_segmentation.py)   | `uv run ${CLAUDE_SKILL_DIR}/scripts/rfm_segmentation.py data.csv --customer customer_id --date order_date --value revenue` |
| [trend_analysis.py](scripts/trend_analysis.py)       | `uv run ${CLAUDE_SKILL_DIR}/scripts/trend_analysis.py data.csv --date date --value revenue --window 30`                    |
| [validate.py](scripts/validate.py)                   | `uv run ${CLAUDE_SKILL_DIR}/scripts/validate.py data.csv`                                                                  |
| [chart_templates.py](scripts/chart_templates.py)     | `uv run ${CLAUDE_SKILL_DIR}/scripts/chart_templates.py data.csv --type bar --x category --y value -o chart.png`            |
...
See [`references/chart-selection.md`](references/chart-selection.md) for the full chart reference.
