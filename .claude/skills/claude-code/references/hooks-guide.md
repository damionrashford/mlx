...
<Tip>
  This guide covers common use cases and how to get started. For full event schemas, JSON input/output formats, and advanced features like async hooks and MCP tool hooks, see the [Hooks reference](hooks-ref.md).
</Tip>
...
Hooks let you run code at key points in Claude Code's lifecycle: format files after edits, block commands before they execute, send notifications when Claude needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](hooks-ref.md#hook-lifecycle).
...
The matcher filters by configuration type: `user_settings`, `project_settings`, `local_settings`, `policy_settings`, or `skills`. To block a change from taking effect, exit with code 2 or return `{"decision": "block"}`. See the [ConfigChange reference](hooks-ref.md#configchange) for the full input schema.
...
Keep the matcher as narrow as possible. Matching on `.*` or leaving the matcher empty would auto-approve every permission prompt, including file writes and shell commands. See the [PermissionRequest reference](hooks-ref.md#permissionrequest-decision-control) for the full set of decision fields.
...
Your script can parse that JSON and act on any of those fields. `UserPromptSubmit` hooks get the `prompt` text instead, `SessionStart` hooks get the `source` (startup, resume, clear, compact), and so on. See [Common input fields](hooks-ref.md#common-input-fields) in the reference for shared fields, and each event's section for event-specific schemas.
...
Other events use different decision patterns. For example, `PostToolUse` and `Stop` hooks use a top-level `decision: "block"` field, while `PermissionRequest` uses `hookSpecificOutput.decision.behavior`. See the [summary table](hooks-ref.md#decision-control) in the reference for a full breakdown by event.
...
For full matcher syntax, see the [Hooks reference](hooks-ref.md#configuration).
...
Run [`/hooks`](hooks-ref.md#the-hooks-menu) in Claude Code to browse all configured hooks grouped by event. To disable all hooks at once, set `"disableAllHooks": true` in your settings file.
...
For full configuration options, see [Prompt-based hooks](hooks-ref.md#prompt-based-hooks) in the reference.
...
For full configuration options, see [Agent-based hooks](hooks-ref.md#agent-based-hooks) in the reference.
...
The endpoint should return a JSON response body using the same [output format](hooks-ref.md#json-output) as command hooks. To block a tool call, return a 2xx response with the appropriate `hookSpecificOutput` fields. HTTP status codes alone cannot block actions.
...
For full configuration options and response handling, see [HTTP hooks](hooks-ref.md#http-hook-fields) in the reference.
...
* `Stop` hooks fire whenever Claude finishes responding, not only at task completion. They do not fire on user interrupts. API errors fire [StopFailure](hooks-ref.md#stopfailure) instead.
...
* [Hooks reference](hooks-ref.md): full event schemas, JSON output format, async hooks, and MCP tool hooks
* [Security considerations](hooks-ref.md#security-considerations): review before deploying hooks in shared or production environments
